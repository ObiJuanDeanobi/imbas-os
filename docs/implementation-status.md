...
| #7 | Lorekeeper | Proposal-first workflow plus guarded managed-block apply with source citations, non-mutating before/after preview, desktop visual line diff, approve-before-apply action guard, apply-time markdown snapshot response, Runledger audit, Command Center proposal summary, and Agent Console transcript-to-artifact/proposal path exist. | Tiered Lorekeeper automation policy implementation for safe auto-apply lanes versus approval-required lanes. |

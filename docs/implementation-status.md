...
| #6 | Android companion | Kotlin/Compose Gradle project, VPS Android SDK/JDK/Gradle wrapper, tabbed private-preview APK, live HTTP status/runledger/proposal fetch with offline demo fallback, desktop-created pairing challenge UI, live pairing completion, Android Keystore-encrypted mobile token storage, and authenticated session revoke are implemented. | QR scan, richer proposal approval POST UX, capture/share actions, and biometric/keychain polish remain. |

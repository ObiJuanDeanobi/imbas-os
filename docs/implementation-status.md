...
| #9 | Memory public 1.0 gate | Adapter, CLI client, optional write-through, status module, live smoke path, agentmemory pattern-harvest note, and explicit MemPalace→Imbas migration plan exist. | Custom Memsocket development is paused as of 2026-05-20 while agentmemory becomes the primary live-memory candidate. Public 1.0 remains blocked until a first-class memory module is proven, documented, backed up/restored, delete/forget tested, and approved by the maintainer. MemPalace remains active as a safety net until the replacement meets migration exit criteria. |
...
- No public 1.0 until the fresh-system gate in [`docs/release/fresh-system-1.0-gate.md`](release/fresh-system-1.0-gate.md) is updated for the chosen memory module and passes: brand-new OpenClaw config, Imbas OS with supported modules, agent adapters, Android companion pairing, backup/restore, security smoke, and human review loop.

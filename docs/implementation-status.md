...
| #4 | Conduit/OpenClaw connector | Conduit loopback, durable JSONL store, OpenClaw shadow connector, events/runs/search/provenance-rich context-packs, run replay timeline, artifact save endpoint, optional Memsocket write-through, and Agent Console v0 steering/action-card surface are implemented. | Live OpenClaw chat/task dispatch, full connector SDKs for Hermes/Codex/Claude Code, and production auth/capability model. |
...
| #6 | Android companion | Kotlin/Compose Gradle scaffold, pairing/session foundation, Conduit endpoint map, read-only live HTTP status/runledger/proposal fetch with offline demo fallback, VPS Android SDK/JDK/Gradle wrapper, successful `assembleDebug`, and tabbed private-preview APK exist. | Secure token storage + live Conduit pairing/action POSTs remain. |

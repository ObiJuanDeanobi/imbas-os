...
| #4 | Conduit/OpenClaw connector | Conduit loopback, durable JSONL store, OpenClaw shadow connector, events/runs/search/context-packs, artifact save endpoint, optional Memsocket write-through, and Agent Console v0 steering surface are implemented. | Live OpenClaw chat/task dispatch, full connector SDKs for Hermes/Codex/Claude Code, and production auth/capability model. |
...
| #7 | Lorekeeper | Proposal-first workflow plus guarded managed-block apply with source citations, Runledger audit, Command Center proposal summary, and Agent Console transcript-to-artifact path exist. | Rich diff/review UI, snapshot-before-apply, and tiered Lorekeeper automation policy implementation for safe auto-apply lanes versus approval-required lanes. |
